"""
mcp_finance.py —— Finance 域 MCP server 入口（stdio）。

把 finance/skill.json 的工具暴露为 MCP tools：飞书 AI Agent 调用 tool ->
Python 执行 finance.main 对应函数 -> 返回 JSON（含 card 字段）-> AI 提取 card 发卡片。
...
server = build_server("finance.main", SKILL_JSON, SERVER_NAME)
